...
async function supabaseDelete(env, table, filter) {
  var res = await fetch(`${env.SUPABASE_URL}/rest/v1/${table}?${filter}`, {
    method: "DELETE",
    headers: {
      "apikey": env.SUPABASE_ANON_KEY,
      "Authorization": `Bearer ${env.SUPABASE_SERVICE_ROLE_KEY}`
...
__name(profileToFrontend, "profileToFrontend");
...
__name(projectToFrontend, "projectToFrontend");
async function supabaseGetAll(env) {
  var [profileRows, botRows, galleryRows, commissionRows, projectRows] = await Promise.all([
...
async function supabasePutAll(env, data) {
  if (data.profile) {
    ...
    var result = await supabaseUpsert(env, "profile", profileRow);
    if (!result) return null;
  }
  if (data.bots) {
    await supabaseDelete(env, "bots", "id=gt.0");
    if (data.bots.length > 0) {
      var botRows = data.bots.map((b) => ({
        icon: b.icon || "",
        name: b.name || "",
      ...
      var result2 = await supabaseUpsert(env, "bots", botRows);
  ...
  if (data.gallery) {
    ...
    if (data.gallery.length > 0) {
      var gallRows = data.gallery.map((g) => ({
        src: g.src || "",
        title: g.title || "",
        description: g.description || "",
        timestamp: g.timestamp || Date.now()
      }));
      var result3 = await supabaseUpsert(env, "gallery", gallRows);
  ...
  if (data.commissions) {
    ...
    if (data.commissions.length > 0) {
      var commRows = data.commissions.map((c) => ({
        src: c.src || "",
        title: c.title || "",
        description: c.description || "",
        timestamp: c.timestamp || Date.now()
      }));
      var result4 = await supabaseUpsert(env, "commissions", commRows);
  ...
  if (data.projects) {
    ...
    if (data.projects.length > 0) {
      var projRows = data.projects.map((p2) => ({
        icon: p2.icon || "",
        name: p2.name || "",
        description: p2.description || "",
        github_repo: p2.githubRepo || "",
        live_demo: p2.liveDemo || "",
        project_folder: p2.projectFolder || ""
      }));
      var result5 = await supabaseUpsert(env, "projects", projRows);
      if (!result5) return null;
    }
  }
  return await supabaseGetAll(env);
}
__name(supabasePutAll, "supabasePutAll");
async function handleDataGet(request, env, origin) {
  var auth = await getAuth(request, env);
  if (!auth) {
    return jsonResponse({ error: "Unauthorized" }, 401, env, origin);
  }
  ...
  var data = await supabaseGetAll(env);
  if (data === null) {
...
async function handleDataPut(request, env, origin) {
  ...
  var result = await supabasePutAll(env, body);
  if (result === null) {
...
async function handleCommissionUpdate(request, env, origin) {
  ...
  await supabaseDelete(env, "commissions", "id=gt.0");
  if (newData.commissions.length > 0) {
    var rows = newData.commissions.map((c) => ({
      src: c.src || "",
      title: c.title || "",
      description: c.description || "",
      timestamp: c.timestamp || Date.now()
    }));
    var result = await supabaseUpsert(env, "commissions", rows);
    if (!result) {
